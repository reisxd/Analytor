...
route.post('/', cors(), async (req, res) => {
  ...
  const browserData = await req.app
    .get('db')
    .collection('browsers')
    .findOne({ token });
  if (!browserData) {
    ...
    const browser = browserData.browsers.find(
      (browser) => browser.name === data.browser.name
    );
  ...
  const analytics = await req.app
    ...
    .findOne({ token, endpoint });
  if (!analytics)
    await req.app
      .get('db')
      .collection('analytics')
      .insertOne({ token, endpoint, visits: 1 });
  else
    req.app
      .get('db')
      .collection('analytics')
      .updateOne({ token, endpoint }, { $inc: { visits: 1 } });
  res.status(204).end();
});
