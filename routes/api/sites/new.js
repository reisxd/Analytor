...
async function sha256(message) {
    return await crypto.createHash('sha256').update(message.toString()).digest('hex');
}

route.post('/', async (req, res) => {
    if (!req.user) return res.status(401).json({ error: 'Unauthorized.' });
    const { url } = req.body;
    const siteUrl = url.match(/^(?:https?:\/\/)?(?:[^@\n]+@)?(?:www\.)?([^:\/\n?]+)/img);
    if (!siteUrl) return res.status(400).json({ error: 'Invalid site url.' });
    const site = await req.app.get('db').collection('sites').findOne({ url: siteUrl[0] });
